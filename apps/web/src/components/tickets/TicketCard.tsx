...
import { Edit3, ExternalLink, GitBranch, MoveRight, Trash2 } from "lucide-react";
...
export function TicketCard({ ticket, allTags, compact = false, variant = "card", onOpen, onOpenInTab, onMove, onDelete, onStatusChange, onDueDateChange, onTagsChange }: TicketCardProps) {
    ...
    return (
      <>
        <div className="md:hidden">
          <TicketCard ticket={ticket} allTags={allTags} compact={compact} onOpen={onOpen} onOpenInTab={onOpenInTab} onMove={onMove} onDelete={onDelete} onStatusChange={onStatusChange} onDueDateChange={onDueDateChange} onTagsChange={onTagsChange} />
        </div>
        <TicketRow ticket={ticket} allTags={allTags} description={description} statusColor={statusColor} ticketClosed={ticketClosed} onOpen={onOpen} onOpenInTab={onOpenInTab} onMove={onMove} onDelete={onDelete} onStatusChange={onStatusChange} onDueDateChange={onDueDateChange} onTagsChange={onTagsChange} />
      </>
    );
  }
  ...
  return (
    <ItemCard
      ...
      extraMenuItems={[
        ...
        ...(onMove ? [{ label: "Verschieben", icon: <MoveRight size={16} />, onClick: () => onMove(ticket) }] : [])
      ]}
...
function TicketRow({ ticket, allTags, description, statusColor, ticketClosed, onOpen, onOpenInTab, onMove, onDelete, onStatusChange, onDueDateChange, onTagsChange }: { ticket: Ticket; allTags?: Tag[]; description: string; statusColor: string; ticketClosed: boolean; onOpen: (ticket: Ticket) => void; onOpenInTab?: (ticket: Ticket) => void; onMove?: (ticket: Ticket) => void; onDelete?: (ticket: Ticket) => void; onStatusChange?: (ticket: Ticket, status: Ticket["status"]) => void | Promise<unknown>; onDueDateChange?: (ticket: Ticket, dueDate: string | null) => void | Promise<unknown>; onTagsChange?: (ticketId: number, tagIds: number[]) => void | Promise<void> }) {
  const overdue = !ticketClosed && isOverdue(ticket.dueDate);
  const closedDate = ticketClosed ? (ticket.resolvedAt ?? ticket.updatedAt) : null;
