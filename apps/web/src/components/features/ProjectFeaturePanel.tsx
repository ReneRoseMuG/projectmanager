    ...
    <section className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-line bg-white px-4 py-3 shadow-sm">
        <div>
          ...
          <p className="text-xs font-semibold text-slate-500">Projektverknüpfungen werden im Feature-Formular gepflegt.</p>
        </div>
            ...
            return (
              <section key={column.value} className={`grid min-h-80 content-start gap-3 rounded-xl border p-4 ${column.wrapperClassName}`}>
                <header className="flex items-center justify-between gap-3 px-1 pb-1">
                  ...
                  <span className="rounded-full bg-white px-2.5 py-0.5 text-xs font-bold text-ink shadow-sm">{columnFeatures.length}</span>
                </header>
...
function FeatureBoardCard({ feature, onOpen }: { feature: Feature; onOpen: (feature: Feature) => void }) {
  return (
    <article
      className="grid cursor-pointer gap-3 rounded-xl border border-line bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-steel-300 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-steel-600/10"
      tabIndex={0}
    ...
    >
      ...
      <p className="line-clamp-3 text-sm text-slate-600">{feature.description || "Keine Kurzbeschreibung"}</p>
      <footer className="flex items-center justify-between gap-3 border-t border-dashed border-line pt-3 text-xs font-semibold text-slate-500">
        <span>{feature.useCaseCount} Use Cases</span>
        <Button
          className="h-8"
          icon={<Edit3 size={15} />}
          onClick={(event) => {
            ...
            {features.map((feature) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-right">
                  <Button className="h-8" icon={<Edit3 size={15} />} onClick={() => onOpen(feature)}>
                    Bearbeiten
                  </Button>
                </td>
              </tr>
