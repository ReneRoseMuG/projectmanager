...
import { addMonths, eachDayOfInterval, endOfISOWeek, endOfMonth, format, isSameDay, isSameMonth, parseISO, startOfISOWeek, startOfMonth } from "date-fns";
...
function MonthDayCell({
  ...
  return (
    <section
      ...
      className={`grid min-h-32 min-w-0 content-start gap-2 border-t border-line p-2 transition ${
        inCurrentMonth ? "bg-white" : "bg-shell/60"
...
export function MonthCalendar({ tasks, initialDate, onTaskClick, onTaskMove }: MonthCalendarProps) {
  const [visibleDate, setVisibleDate] = useState(() => parseISO(initialDate ?? toDateKey(new Date())));
  const [activeTask, setActiveTask] = useState<Task | null>(null);
  const days = useMemo(() => monthDays(visibleDate), [visibleDate]);
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-line bg-white shadow-sm">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-line bg-white px-4 py-3">
        <Button size="sm" variant="secondary" icon={<ChevronLeft size={15} />} onClick={() => setVisibleDate((current) => addMonths(current, -1))}>
          Zurück
        </Button>
        <div className="flex min-w-0 flex-col items-center gap-1 text-center">
          <h2 className="truncate text-lg font-bold text-ink">{format(visibleDate, "MMMM yyyy", { locale: de })}</h2>
          <Button size="sm" variant="secondary" icon={<CalendarDays size={15} />} onClick={() => setVisibleDate(new Date())}>
            Heute
          </Button>
        </div>
        <Button size="sm" variant="secondary" icon={<ChevronRight size={15} />} onClick={() => setVisibleDate((current) => addMonths(current, 1))}>
          Weiter
        </Button>
      </header>
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={(event) => void handleDragEnd(event).catch(() => undefined)} onDragCancel={() => setActiveTask(null)}>
        <div className="overflow-x-auto">
          <div className="grid min-w-[720px] grid-cols-7 border-b border-line bg-shell">
