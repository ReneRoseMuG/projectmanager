...
import { AlignCenter, AlignLeft, AlignRight, Bold, Heading1, Heading2, Italic, List, ListOrdered, Pencil, Text, Underline as UnderlineIcon } from "lucide-react";
...
export function RichTextInlineField({ value, onChange, placeholder, readOnly = false, className = "", testIdPrefix }: RichTextInlineFieldProps) {
  ...
  return (
    <div className={cn("relative group", className)}>
      {isEditing ? (
        <RichTextInlineEditor value={value ?? ""} originalValue={originalValue} placeholder={placeholder} clickPosition={clickPosition} testIdPrefix={testIdPrefix} onCommit={handleCommit} onCancel={handleCancel} />
      ) : hasContent ? (
        <div
          className={cn(
        ...
        <div
          className={cn("rounded-md px-3 py-2 text-sm italic text-slate-500 transition-colors", !readOnly && "cursor-text hover:bg-shell/80")}
          data-testid={testIdPrefix ? `${testIdPrefix}-view` : undefined}
          onClick={readOnly ? undefined : handleActivate}
        >
          {placeholder}
...
function RichTextInlineEditor({ value, originalValue, placeholder, clickPosition, testIdPrefix, onCommit, onCancel }: RichTextInlineEditorProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "rich-text-surface max-w-none"
      }
    },
