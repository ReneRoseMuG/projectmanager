      ...
      {options.map((option) => {
        ...
        return (
          <button
            key={option.value}
            type="button"
            className={`flex h-10 items-center justify-between rounded-md border px-3 text-sm font-semibold transition ${selected ? activeClasses[color] : inactiveClasses[color]}`}
            onClick={() => onChange(option.value)}
          >
            <span>{option.label}</span>
            {selected ? <Check size={16} className={checkClasses[color]} /> : null}
          </button>
        );
      })}
    </div>
