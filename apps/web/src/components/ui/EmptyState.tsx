...
export function EmptyState({ icon, title, body, tone = "neutral", variant = "default", actions, className = "" }: EmptyStateProps) {
  if (variant === "first-run") {
    return (
      <section className={`rounded-lg bg-gradient-to-br from-steel-700 to-steel-900 p-8 text-center text-white shadow-steel ${className}`}>
        <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-lg bg-white text-steel-700">{icon}</span>
        ...
        {actions?.length ? (
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            {actions.map((action) => (
              <Button key={action.label} variant="inverted" icon={action.icon} onClick={action.onClick}>
                {action.label}
              </Button>
            ))}
          </div>
  ...
  return (
    <section className={`rounded-lg border p-8 text-center ${shell} ${className}`}>
      <span className={`mx-auto flex h-12 w-12 items-center justify-center rounded-lg ${toneClass[tone]}`}>{icon}</span>
      <h3 className="mt-3 text-base font-bold text-ink">{title}</h3>
      {body ? <div className="mx-auto mt-1 max-w-lg text-sm leading-6 text-steel-600">{body}</div> : null}
      {actions?.length ? (
