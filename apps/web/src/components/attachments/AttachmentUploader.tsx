...
export function AttachmentUploader({ onUpload, size = "default" }: AttachmentUploaderProps) {
  ...
  return (
    ...
    >
      ...
      />
      <div className={`flex items-center justify-center rounded-lg bg-steel-700 text-white shadow-steel-icon ${compact ? "h-10 w-10" : "h-14 w-14"}`}>
        <Upload size={compact ? 18 : 22} />
      </div>
      <div>
        <h3 className={`${compact ? "text-sm" : "text-base"} font-bold text-ink`}>Dateien hier ablegen</h3>
        <p className="text-sm text-steel-500">oder über den Button auswählen - max. 25 MB pro Datei</p>
      </div>
      <div>
      ...
      {uploading.length > 0 ? <p className="text-xs text-steel-500">{uploading.join(", ")}</p> : null}
    </div>
  );
}
