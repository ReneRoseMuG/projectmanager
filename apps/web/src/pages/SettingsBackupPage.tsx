...
function syncReadinessTone(
  readiness: DumpIncrementalSyncPreviewResult["transferReadiness"],
) {
  if (readiness === "ready") return "fern";
  if (readiness === "warning") return "mustard";
  return "crimson";
...
export function SettingsBackupPage() {
  ...
  const [syncResult, setSyncResult] =
    useState<DumpIncrementalSyncResult | null>(null);
  const [syncPreview, setSyncPreview] =
    useState<DumpIncrementalSyncPreviewResult | null>(null);
  const [syncApplyResult, setSyncApplyResult] =
    useState<DumpIncrementalSyncApplyResult | null>(null);
  ...
  async function handleIncrementalSyncImport() {
    ...
    try {
      ...
      const approved = await confirm({
        ...
        body: (
          <span>
            Der Remote-Stand {nextPreview.dumpId} überschreibt die lokalen
            Daten. Vor dem Import wird automatisch eine lokale Sicherung
            angelegt.
          </span>
        ),
        severity: "danger",
      ...
      const approved = await confirm({
        ...
        body: (
          <span>
            ...
            dem Import wird automatisch eine lokale Sicherung angelegt.
          </span>
        ),
        severity: "danger",
        confirmLabel: "Ja",
  ...
  const latestSyncDetail = syncResult
    ? `Tabellen: ${syncResult.tablesUpdated ? "aktualisiert" : "unverändert"} · Hochgeladen: ${syncResult.filesUploaded} · Gelöscht: ${syncResult.filesDeleted} · Remote-Dateien: ${syncResult.totalRemoteFiles}`
    : "Noch keine Synchronisation in dieser Sitzung";
      ...
      <div className="flex min-h-0 w-full flex-1 flex-col gap-6 overflow-auto bg-white p-5">
        ...
        <section className="rounded-lg border border-line bg-white p-4 shadow-sm">
          <div className="grid divide-y divide-line">
            <div className="flex flex-wrap items-center justify-between gap-4 pb-4">
              <div className="grid min-w-0 gap-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {backupStatus.status && (
                    <Badge
                      tone={backupStatus.status.ready ? "fern" : "crimson"}
                    >
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="grid min-w-0 gap-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {remoteStatus.status && (
                      <Badge
                        tone={remoteStatus.status.ready ? "fern" : "crimson"}
                      >
                        {remoteStatus.status.ready ? "bereit" : "nicht bereit"}
                      </Badge>
                    )}
                  </div>
                  <p className="text-sm text-steel-600">
                  ...
                  {syncResult && syncResult.filesDeleteFailed > 0 && (
                    <p className="rounded-md border border-mustard/25 bg-mustard/10 p-2 text-sm text-mustard-dark">
                      Nicht gelöscht: {syncResult.filesDeleteFailed}
                    </p>
                  )}
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    loading={incrementalSync.syncing}
                    disabled={!canWriteDumps}
                  ...
                  <Button
                    ...
                    loading={
                      incrementalSync.previewing || incrementalSync.applying
                    }
                    disabled={!canWriteDumps}
                    onClick={() => void handleIncrementalSyncImport()}
                  >
                    Sync importieren
                  </Button>
                </div>
            ...
            {syncPreview && (
              <div className="mt-4 grid gap-3 border-t border-line pt-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <Badge
                    tone={syncReadinessTone(syncPreview.transferReadiness)}
                  >
                    {syncPreview.transferReadiness}
                  </Badge>
                </div>
                ...
                {(syncPreview.warnings.length > 0 ||
                  syncPreview.blockingIssues.length > 0) && (
            ...
            {syncApplyResult && (
              <div className="mt-4 rounded-md border border-fern/20 bg-fern/10 p-3 text-sm text-fern">
                Sync-Import {syncApplyResult.importStatus}. Tabellen:{" "}
                {syncApplyResult.tablesRestored}. Verifikation:{" "}
                {syncApplyResult.verificationPassed
                  ? "bestanden"
                  : "fehlgeschlagen"}
                .
              </div>
