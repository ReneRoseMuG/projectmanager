...
import { Check, Download, FileText, FolderArchive, Inbox, Layers, Pencil, Trash2, X } from "lucide-react";
...
export function DocumentsPage() {
  ...
  const run = useCallback(
    async (action: () => Promise<unknown>, successTitle?: string) => {
      try {
        ...
        showToast({ tone: "error", title: "Aktion fehlgeschlagen", message: toQueryError(mutationError) ?? "Unbekannter Fehler" });
      }
    },
    [showToast]
  );
  ...
  const startEdit = (id: number, currentName: string, kind: "folder" | "category") => {
  ...
  const saveFolderName = (folder: AttachmentFolder) => {
    ...
    if (name && name !== folder.name) {
      void run(() => updateFolder(folder.id, { name, expectedVersion: folder.version }), "Sammlung umbenannt");
    }
    cancelEdit();
  };
  ...
  const saveCategoryName = (category: AttachmentCategory) => {
    ...
    if (name && name !== category.name) {
      void run(() => updateCategory(category.id, { name, expectedVersion: category.version }), "Kategorie umbenannt");
    }
    cancelEdit();
  };
  ...
  const handleDeleteCategory = (category: AttachmentCategory) => {
    if (window.confirm(`Kategorie „${category.name}" löschen? Die Zuordnung wird von allen Dokumenten entfernt, die Dokumente bleiben erhalten.`)) {
  ...
  const editRow = (onSave: () => void) => (
    <div className="flex items-center gap-1 px-1">
      ...
      />
      <button type="button" onClick={onSave} className="rounded-md p-1 text-emerald-600 hover:bg-emerald-50" title="Speichern">
        <Check size={14} />
      </button>
      <button type="button" onClick={cancelEdit} className="rounded-md p-1 text-steel-500 hover:bg-steel-50" title="Abbrechen">
        <X size={14} />
      </button>
    </div>
  );
    ...
    <div className="flex min-h-0 flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        <aside className="flex flex-col gap-4">
          <div className="flex flex-col gap-1 rounded-lg border border-line bg-white p-2">
            <span className="px-2 py-1 text-xs font-medium uppercase tracking-wide text-steel-400">Sammlungen</span>
            ...
            {folders.map((folder) =>
              editingFolderId === folder.id ? (
                <div key={folder.id}>{editRow(() => saveFolderName(folder))}</div>
              ) : (
                <div key={folder.id} className="group flex items-center gap-1" style={{ paddingLeft: folder.parentId ? 12 : 0 }}>
                  <button
                    type="button"
                    onClick={() => setFolderScope(folder.id)}
                    className={`flex min-w-0 flex-1 items-center gap-2 rounded-md px-3 py-2 text-left text-sm ${
                      folderScope === folder.id ? "bg-steel-100 text-ink" : "text-steel-600 hover:bg-steel-50"
                    }`}
                  >
                  ...
                  {canWrite ? (
                    <div className="flex opacity-0 transition-opacity group-hover:opacity-100">
                      <button type="button" onClick={() => startEdit(folder.id, folder.name, "folder")} className="rounded-md p-1 text-steel-400 hover:bg-steel-50 hover:text-ink" title="Umbenennen">
                        <Pencil size={14} />
                      </button>
                      <button type="button" onClick={() => handleDeleteFolder(folder)} className="rounded-md p-1 text-steel-400 hover:bg-rose-50 hover:text-rose-600" title="Löschen">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ) : null}
            ...
            {canWrite ? (
              <form
                ...
                onSubmit={(event) => {
                  ...
                  void run(() => createFolder({ name, parentId: typeof folderScope === "number" ? folderScope : null }), "Sammlung angelegt");
                  setNewFolderName("");
                }}
              >
                <input
          ...
          <div className="flex flex-col gap-1 rounded-lg border border-line bg-white p-2">
            <span className="px-2 py-1 text-xs font-medium uppercase tracking-wide text-steel-400">Kategorien</span>
            {categories.length === 0 ? <span className="px-3 py-1 text-xs text-steel-400">Noch keine Kategorien.</span> : null}
            {categories.map((category) =>
              editingCategoryId === category.id ? (
                <div key={category.id}>{editRow(() => saveCategoryName(category))}</div>
              ) : (
                <div key={category.id} className="group flex items-center gap-1">
                  ...
                  {canWrite ? (
                    <div className="flex opacity-0 transition-opacity group-hover:opacity-100">
                      <button type="button" onClick={() => startEdit(category.id, category.name, "category")} className="rounded-md p-1 text-steel-400 hover:bg-steel-50 hover:text-ink" title="Umbenennen">
                        <Pencil size={14} />
                      </button>
                      <button type="button" onClick={() => handleDeleteCategory(category)} className="rounded-md p-1 text-steel-400 hover:bg-rose-50 hover:text-rose-600" title="Löschen">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ) : null}
            ...
            {canWrite ? (
              <form
                ...
                onSubmit={(event) => {
                  ...
                  void run(() => createCategory({ name }), "Kategorie angelegt");
                  setNewCategoryName("");
                }}
              >
                <input
          ...
          {canWrite ? <AttachmentUploader onUpload={(file) => run(() => uploadDocument(file, uploadFolder), "Dokument hochgeladen")} size="sm" /> : null}
        </aside>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {canDelete ? (
                        <button
                          type="button"
                          onClick={() => {
                            if (window.confirm(`Dokument „${documentTitle(document)}" endgültig löschen?`)) {
      ...
      {selected ? (
        <DocumentDetailPanel
          ...
          onSetTags={(next) => run(() => setTags(selected.id, next.map((tag) => tag.id)), undefined)}
          onAssignCategory={(categoryId) => run(() => assignCategory(selected.id, categoryId))}
          onRemoveCategory={(categoryId) => run(() => removeCategory(selected.id, categoryId))}
          onAddToFolder={(folderId) => run(() => addToFolder(folderId, selected.id), "In Sammlung einsortiert")}
          onRemoveFromFolder={(folderId) => run(() => removeFromFolder(folderId, selected.id), "Aus Sammlung entfernt")}
          onSaveMetadata={(input) => run(() => updateMetadata(selected.id, { ...input, expectedVersion: selected.version }), "Gespeichert")}
        />
      ) : null}
    </div>
  );
...
function DocumentDetailPanel({
  ...
  const availableFolders = folders.filter((folder) => !assignedFolderIds.has(folder.id));
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end bg-black/20" onClick={onClose}>
      <div className="flex h-full w-full max-w-md flex-col gap-5 overflow-auto bg-white p-6 shadow-steel" onClick={(event) => event.stopPropagation()}>
        ...
        <section className="flex flex-col gap-2">
          <span className="text-xs font-medium uppercase tracking-wide text-steel-400">Sammlungen</span>
          <div className="flex flex-wrap gap-1">
            {documentFolders.map((folder) => (
              <span key={folder.id} className="flex items-center gap-1 rounded-md bg-steel-100 px-2 py-0.5 text-xs text-ink">
                <FolderArchive size={12} />
                {folder.name}
                {canWrite ? (
                  <button type="button" onClick={() => onRemoveFromFolder(folder.id)} className="text-steel-500 hover:text-rose-600" title="Aus Sammlung entfernen">
                    <X size={12} />
                  </button>
                ) : null}
              </span>
            ))}
            {documentFolders.length === 0 ? <span className="text-xs text-steel-400">In keiner Sammlung.</span> : null}
          </div>
        ...
        <section className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {categories.length === 0 ? <span className="text-xs text-steel-400">Kategorien werden links verwaltet.</span> : null}
          </div>
        </section>
      </div>
    </div>
