...
- **Breite:** Über `widthPx` dynamisch, sonst gilt die feste responsive Breite als Fallback. Rechts stammt der Wert aus dem Zieh-Griff (`onResizeStart`), links wird er **inhaltsgesteuert** aus dem längsten Eintrag berechnet (`computePanelWidth`, Korridor 280–460 px; Textmessung über `utils/textMeasure`). Einträge werden dabei **nicht** gekürzt (`truncate`), sondern brechen um (`break-words`) — Namen bleiben immer vollständig lesbar.
- **Lange Zuordnungslisten:** Jede Liste im Panel (z. B. Sammlungen, Kategorien) scrollt in einem eigenen begrenzten Bereich (`max-h-[40vh] overflow-y-auto`). Abschnittsüberschrift und „Neu…"-Formular bleiben dabei stehen, damit sie bei vielen Einträgen erreichbar sind.
- **Ablageziele:** Zeilen von Zuordnungslisten sind Drop-Ziele (`@dnd-kit`, `useDroppable`, `disabled` ohne Schreibrecht); Hover-Zustand `bg-white/10 ring-1 ring-inset ring-white/35`. Filterzeilen wie „Alle Dokumente" oder „Nicht einsortiert" sind **keine** Ablageziele — sie sind Filter, keine Zuordnung.
...
**Verboten:** Detailinhalte als Modal-Overlay, wenn ein seiten-internes Panel vorgesehen ist; ein Panel ohne Collapse-Toggle oder ohne Zustands-Persistenz; Sticky-Panels als Flex-/Grid-Items ohne `self-start` (verhindert das Kleben); ein Steuerelement, dessen Klick je nach verborgenem Auswahlzustand einmal filtert und einmal schreibt (Doppelmodus) — Filtern und Zuweisen sind getrennte Gesten.
...
- **Interaktion:** Einfachklick togglet die Mehrfachauswahl, Doppelklick öffnet die Großansicht, Ziehen ab 6 px startet die Zuweisung (`PointerSensor` mit `activationConstraint: { distance: 6 }` — darunter bleibt es ein Klick).
- **Zuweisung per Drag & Drop:** Kacheln werden auf einen Sammlungs- oder Kategorie-Eintrag der linken Navigation gezogen (`@dnd-kit`; `DndContext` auf Seitenebene, weil er Panel und Grid umspannt; `DragOverlay` zeigt die Anzahl). Wird eine **markierte** Kachel gezogen, wandert die gesamte Auswahl mit, sonst nur die gezogene. Ein **Klick** auf denselben Eintrag filtert dagegen immer. Die Auswahl bleibt nach einer Zuweisung bestehen; Zip-Download und Auswahl-Aufheben liegen in einer Auswahl-Leiste über dem Grid. **Gelöst** werden Zuordnungen in der Großansicht, nicht im Grid.
- **Toolbar-Aufteilung:** Das Suchfeld bekommt eine feste Breite (kein `flex-1`, sonst verdrängt es die Filter), die Filter-Dropdowns eine lesbare Mindestbreite, der Kachelgrößen-Wähler sitzt rechts (`ml-auto`).
...
- **Auswahl und Filter:** Ausgefilterte Dokumente verlieren ihre Markierung, sobald die Liste **vollständig geladen** ist. Das Gate dafür ist `isComplete` aus `useProgressiveList` — **nicht** `loading`/`loadingMore`, die zwischen zwei Blöcken des progressiven Nachladens beide `false` sind. Eine Auswahl darf nie auf unsichtbare Kacheln zeigen: Der Zähler wäre unehrlich, die Markierung ließe sich nicht mehr abwählen, und ein Drag zöge sie stillschweigend mit.
- **Upload-Kontext:** Die geöffnete Sammlung und die gewählte Kategorie sind das **Ziel** des Uploads (`?folder=`, `?category=`). Label-, Typ-, Endungsfilter und Suche sind dagegen bloße **Einschränkungen**; sie werden nach erfolgreichem Upload geleert, sonst wäre die neue Datei sofort wieder unsichtbar. Weil hier ein Ansichtszustand eine Schreibwirkung auslöst, benennt der Erfolgs-Toast das Ziel ausdrücklich.
- **Mehrfach-Upload:** Nachgeladen wird **einmal je Upload-Vorgang**, nicht je Datei — `AttachmentUploader` meldet den Abschluss über `onBatchComplete`, die Upload-Mutation invalidiert bewusst nicht. Eine Invalidierung je Datei würde die progressiv geladene Liste jedes Mal vollständig neu holen und (weil `mutateAsync` auf `onSuccess` wartet) den nächsten Upload blockieren. Ebenso fasst **ein** Toast die Erfolge zusammen; Fehler bleiben je Datei sichtbar.
