...
test.describe("Feature-Routen und Detailformular", () => {
  test("Feature erstellen: Plus-Button navigiert auf Create-Detailseite und Speichern schließt", async ({
    page,
    request,
  }) => {
    ...
    try {
      ...
      await fillRichText(
        ...
        "E2E Feature-Neuanlage Inhalt vollständig",
      );
  ...
  test("Feature bearbeiten: Speichern schließt auf die Rücksprung-Route und aktualisiert die Übersicht", async ({
    page,
    request,
  }) => {
    ...
    try {
      ...
      await Promise.all([
        page.waitForResponse(
          (response) =>
            response.url().includes(`/api/features/${feature.id}`) &&
  ...
  test.skip("Feature-Projekte-Tab: Doppelklick und Bearbeiten öffnen das Projekt-Detailformular per Route", async ({
    ...
    try {
      ...
      await clickItemAction(
        formPage(page, "Feature bearbeiten"),
        project.name,
        "Bearbeiten",
      );
      await expect(page).toHaveURL(new RegExp(`/projects/${project.id}`));
      await expect(
        formPage(page, "Projekt bearbeiten").locator("input[required]").first(),
  ...
  test("Feature-Use-Cases-Tab: Doppelklick und Bearbeiten öffnen das Use-Case-Detailformular per Route", async ({
    ...
    try {
      ...
      await authenticatedGoto(page, `/features/${feature.id}`);
      await formPage(page, "Feature bearbeiten")
        .getByRole("button", { name: /Use Cases/ })
        .click();
      await clickItemAction(
        formPage(page, "Feature bearbeiten"),
        useCase.title,
      ...
      await expect(page).toHaveURL(new RegExp(`/use-cases/${useCase.id}`));
      await expect(
        formPage(page, "Use Case bearbeiten").locator("input[required]").nth(0),
      ).toHaveValue(useCase.title);
    } finally {
      await deleteFeature(request, feature.id);
    }
  });

  test("Feature löschen: Delete und Confirm entfernen die Karte aus der Übersicht", async ({
    page,
    request,
  }) => {
  ...
  test("Board- und Listenansicht zeigen echte Feature-Daten", async ({
    page,
    request,
  }) => {
