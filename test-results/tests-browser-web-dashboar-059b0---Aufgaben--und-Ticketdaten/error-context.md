...
- Name: tests\browser\web\dashboard.spec.ts >> Dashboards >> zeigt Projekt-Dashboard mit echten Meilenstein-, Aufgaben- und Ticketdaten
- Location: tests\browser\web\dashboard.spec.ts:45:3

# Error details

```
Error: apiRequestContext.post: connect ECONNREFUSED 127.0.0.1:3101
Call log:
  - → POST http://127.0.0.1:3101/api/auth/login
    - user-agent: Playwright/1.60.0 (x64; windows 10.0) node/24.12
    - accept: */*
...
```ts
  ...
  7   | 
  8   | export const apiBaseUrl =
  9   |   process.env.PLAYWRIGHT_API_BASE_URL ?? "http://127.0.0.1:3101/api";
  10  | 
